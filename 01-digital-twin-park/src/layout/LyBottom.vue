...
import { getDefaultButtonId, getDefaultChildButtonId, getButtonInfo } from '../config/videoConfig';
...
var isClicked = false;
var gradientElement = null;

// 修改渐变颜色的函数，提升到组件作用域以便在 activeItem 中使用
...
onMounted(() => {
  ...
  // 页面加载完成后，默认激活园区总览按钮
  // 只激活按钮样式和切换视频，不调用 mapApi 相关操作（因为 mapApi 可能还未初始化）
  setTimeout(() => {
    const defaultButtonId = getDefaultButtonId();
    if (defaultButtonId) {
      // 激活按钮样式
      activeItem(defaultButtonId, 'listActive');
      // 切换视频（不依赖 mapApi）
      if (window.switchVideo) {
        window.switchVideo(defaultButtonId);
      }
      // 注意：不在这里调用 showBtn，因为 showBtn 需要 mapApi 已初始化
      // 如果需要执行 mapApi 相关操作，可以等待 mapApi 初始化完成后再调用
    }
  }, 100);
});

// 检查按钮是否已经选中
function isButtonAlreadyActive(id) {
  switch (id) {
    case 'childBtnOne': {
      const btnElement = document.getElementById('btn_one');
      return btnElement && btnElement.classList.contains('listActive');
    }
    case 'childBtnTwo': {
      const btnElement = document.getElementById('btn_two');
      return btnElement && btnElement.classList.contains('listActive');
    }
    case 'childBtnThree': {
      const btnElement = document.getElementById('btn_three');
      const isMainActive = btnElement && btnElement.classList.contains('listActive');
      if (!isMainActive) return false;
      // 检查默认子按钮是否已激活
      const firstSecurityChild = getDefaultChildButtonId('btn_three');
      if (firstSecurityChild) {
        const childElement = document.getElementById(firstSecurityChild);
        return childElement && childElement.classList.contains('btn_children_active');
      }
      return true;
    }
    case 'childBtnFour': {
      const btnElement = document.getElementById('btn_four');
      const isMainActive = btnElement && btnElement.classList.contains('listActive');
      if (!isMainActive) return false;
      // 检查默认子按钮是否已激活
      const firstDeviceChild = getDefaultChildButtonId('btn_four');
      if (firstDeviceChild) {
        const childElement = document.getElementById(firstDeviceChild);
        return childElement && childElement.classList.contains('btn_children_active');
      }
      return true;
    }
...
// 主按钮
function showBtn(id) {
  // 如果按钮已经处于选中状态，直接返回，避免重复加载
  if (isButtonAlreadyActive(id)) {
    return;
  }
  ...
  // 先执行不依赖 mapApi 的操作：UI 更新和视频切换
  let buttonId = null; // 用于视频切换的按钮ID
  ...
  switch (id) {
    ...
    case 'childBtnThree':
      ...
      // 获取默认子按钮ID，如果存在则使用子按钮，否则使用父按钮
      const firstSecurityChild = getDefaultChildButtonId('btn_three');
      if (firstSecurityChild) {
        buttonId = firstSecurityChild;
        // 延迟激活子按钮，确保 DOM 已渲染
        requestAnimationFrame(() => {
          setTimeout(() => {
            activeItem(firstSecurityChild, 'btn_children_active');
          }, 10);
        });
      } else {
        buttonId = 'btn_three';
    ...
    case 'childBtnFour':
      ...
      const firstDeviceChild = getDefaultChildButtonId('btn_four');
      if (firstDeviceChild) {
        buttonId = firstDeviceChild;
        // 延迟激活子按钮，确保 DOM 已渲染
        requestAnimationFrame(() => {
          setTimeout(() => {
            activeItem(firstDeviceChild, 'btn_children_active');
          }, 10);
        });
      } else {
        buttonId = 'btn_four';
  ...
  // 执行视频切换（不依赖 mapApi）
  if (buttonId && window.switchVideo) {
    window.switchVideo(buttonId);
  } else if (buttonId && !window.switchVideo) {
    console.error('window.switchVideo 不存在！无法切换视频');
  }
  
  // 执行需要 mapApi 的操作（如果 mapApi 已初始化）
  if (!window.mapApi) {
    return;
  }
  
  // 根据不同的按钮执行对应的 mapApi 操作
  switch (id) {
    case 'childBtnOne':
    ...
    case 'childBtnThree':
      ...
      window.mapApi.stopRotation();
      clearRecording();
      // 执行默认子按钮的功能（激活已经在上面完成了）
      const firstSecurityChildId = getDefaultChildButtonId('btn_three');
      if (firstSecurityChildId) {
        bindFunc(firstSecurityChildId);
      }
      break;
    case 'childBtnFour':
      window.mapApi.removeViewIndoors();
      ...
      window.mapApi.addFence(FenceData);
      // 执行默认子按钮的功能（激活已经在上面完成了）
      const firstDeviceChildId = getDefaultChildButtonId('btn_four');
      if (firstDeviceChildId) {
        bindFunc(firstDeviceChildId);
      }
      break;
    case 'home':
      window.mapApi.removeViewIndoors();
...
function bindFunc(id) {
  // 检查子按钮是否已经选中，如果已选中则直接返回，避免重复加载
  const childElement = document.getElementById(id);
  if (childElement && childElement.classList.contains('btn_children_active')) {
    return;
  }
...
function getButtonName(buttonId) {
  ...
  return buttonInfo ? buttonInfo.name : '';
}
...
function activeItem(id, className) {
  if (id === 'home') {
    ...
    // 设置 home 按钮为选中状态，应用与 hover 相同的颜色
    isClicked = true;
    changeGradientColor('rgb(240, 160, 10)', '#ffffff');
  } else {
    // 其他按钮被选中时，重置 home 按钮状态
    isClicked = false;
    changeGradientColor('#ffffff', '#ffffff');
